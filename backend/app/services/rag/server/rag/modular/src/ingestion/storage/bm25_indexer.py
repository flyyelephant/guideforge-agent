...
dense -> 写入 vector store
sparse -> 写入 BM25 index
image -> 写入 image storage
三者最终通过同一套稳定 chunk_id / vector_id 对齐
"""
...
class BM25Indexer:
    ...
    def build(
        ...
        for term, df in doc_freq.items():
            ...
            index[term] = {     #每个 term 都有一个 idf、df 和 posting 列表，term即词项，一个词出现在哪些 chunk 里
                "idf": idf,
                "df": df,
                "postings": postings
            }
    ...
    def _calculate_idf(self, num_docs: int, df: int) -> float:      #IDF 计算公式，输入是文档总数和包含该词的文档数，分数越高表示这个词越稀有，越重要
        """Calculate IDF using BM25 formula.
