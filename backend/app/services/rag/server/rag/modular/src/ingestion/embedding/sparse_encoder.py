...
class SparseEncoder:
    ...
    def encode(
        ...
        for i, chunk in enumerate(chunks):          #稀疏统计的输入是按chunk跑的
            # Validate chunk text
            ...
            terms = self._tokenize(chunk.text)          #将 chunk 文本切分成词项列表，进行基本清洗和过滤 chunk拆词
            term_frequencies = Counter(terms)
            
            # Build statistics dict
            stat_dict = {
                "chunk_id": chunk.id,       #统计表是属于哪个 chunk
                "term_frequencies": dict(term_frequencies),  #每个词项在这个 chunk 中出现的次数
                "doc_length": len(terms),           #这个 chunk 中的总词项数（文档长度）
                "unique_terms": len(term_frequencies),      #这个 chunk 中的不同词项数（词汇量）
            }
    ...
    def _tokenize(self, text: str) -> List[str]:
        ...
        Uses jieba for Chinese text segmentation and regex for English.使用jieba进行中文文本分词，使用正则表达式进行英文文本分词。
        ...
        raw_tokens = jieba.lcut(text)           #jieba分词策略
        ...
        if self.lowercase:
            tokens = [t.lower() for t in tokens]    #大小写统一
        
        # Filter by minimum length
        terms = [t for t in tokens if len(t) >= self.min_term_length]       #最小长度过滤，晒噪声单词
