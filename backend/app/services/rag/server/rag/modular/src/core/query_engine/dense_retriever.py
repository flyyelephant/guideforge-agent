...
"""
#把用户的 query 变成向量，然后去向量库里找“语义上最相近”的 chunks出来，返回给用户
...
class DenseRetriever:
    ...
    def __init__(
        ...
        embedding_client: Optional[BaseEmbedding] = None,       #把 query 变成向量
        vector_store: Optional[BaseVectorStore] = None,         #拿 query 向量去做相似度搜索
        default_top_k: int = 10,                                #默认返回多少个结果，用户可以覆盖这个值，或者在 settings 里配置
    ) -> None:
    ...
    def retrieve(
        ...
        try:
            raw_results = self.vector_store.query(              #去向量库查相似向量，拿着 query_vector，去和库里所有 chunk 的向量比较
                vector=query_vector,
