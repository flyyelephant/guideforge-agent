...
class DenseEncoder:
    ...
    def __init__(
        ...
        if min_batch_size > batch_size:
            raise ValueError(
                f"min_batch_size ({min_batch_size}) cannot exceed batch_size ({batch_size})"
            )
    ...
    def encode(
        ...
        for batch_start in range(0, len(texts), self.batch_size):
            ...
            try:
                batch_vectors = self._encode_with_retry(
                    texts=batch_texts,
                    trace=trace,
    ...
    def _encode_with_retry(
        ...
        """Encode a batch and recursively split it if the provider rejects it."""
        try:
            ...
            if len(texts) <= self.min_batch_size:
                raise RuntimeError(
                    f"Embedding request failed for batch {batch_label or 'single'} "
                    f"with size={len(texts)}: {exc}"
                ) from exc

            split_at = max(len(texts) // 2, self.min_batch_size)
            left = texts[:split_at]
            right = texts[split_at:]

            if self.retry_delay_seconds > 0:
                time.sleep(self.retry_delay_seconds)

            left_vectors = self._encode_with_retry(
                texts=left,
                trace=trace,
                batch_label=f"{batch_label}:L",
            )
            right_vectors = self._encode_with_retry(
                texts=right,
                trace=trace,
