"""模块化RAG MCP服务器的摄取管道编排器。

此模块实现了主要流程，该流程负责协调整个文档摄取流程：
    1. File Integrity Check (SHA256 skip check) 受否已经处理过
    2. Document Loading (PDF → Document)        把原始文件变成 Document
    3. Chunking (Document → Chunks)             把Document切成Chunk
...
class IngestionPipeline:
    ...
    def run(
        ...
        try:
            ...
            # BatchProcessor may drop only the chunks that still fail after
            # recursive retries, so storage must follow the successful subset.
            ...
            logger.info(f"  Encoded chunks: {len(processed_chunks)} / {len(chunks)}")
            ...
            stages["encoding"] = {
                "encoded_chunk_count": len(processed_chunks),
            ...
            if trace is not None:
                ...
                chunk_details = []
                for idx, c in enumerate(processed_chunks):
                    detail: dict = {
                        "chunk_id": c.id,
                        "char_len": len(c.text),
                    }
            ...
            vector_ids = self.vector_upserter.upsert(processed_chunks, dense_vectors, trace)
